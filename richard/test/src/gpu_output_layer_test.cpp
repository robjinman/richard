...
Vector cpuOutputLayerTrainForward(size_t layerSize, const Matrix& W, const Vector& B,
  const Vector& inputs) {
  ...
  cpu::OutputLayer layer(json, inputs.size());
...
TEST_F(GpuOutputLayerTest, trainForward) {
  ...
  nlohmann::json json;
  json["size"] = outputSize;
  json["learnRate"] = 0.1;
  json["learnRateDecay"] = 1.0;

  gpu::OutputLayer layer(*gpu, json, layerInputSize, miniBatchSize);
  ...
  Vector expectedA = cpuOutputLayerTrainForward(outputSize, W, B, inputs);
